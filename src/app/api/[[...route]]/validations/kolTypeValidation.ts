import type { KolTypeInput } from "@/app/types/kolTypeTypes";

export function validateKolType(data: KolTypeInput, existingMinFollowers?:number) {
    ...
    if ("max_followers" in data) {
        if (data.max_followers !== null && typeof data.max_followers !== "number") {
            return { valid: false, message: "max_followers must be a number or null." };
        }

        const min = "min_followers" in data
            ? data.min_followers
            : existingMinFollowers;
